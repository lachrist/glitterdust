...
store1 :: Element1 -> Store Element1 -> (Value1, Store Element1)
store1 χ σ = (Address1 α, Map.insert α χ σ)
...
instance System Value2 Element2 where
  ...
  applyBuiltin "cons" (ν:ν':_) σ κ = kontinue κ (Address2 α, Map.insert α (Cons2 (ν, ν')) σ)
    where α = maybe 0 ((+1) . fst) (Map.lookupMax σ)
...
store3 :: Element3 -> Store Element3 -> (Value3, Store Element3)
store3 χ@(Cons3 _) σ = (Address3 α, Map.insert α χ σ)
  where α = maybe 0 ((+1) . fst) (Map.lookupMax σ)
store3 χ σ = maybe (Address3 α, Map.insert α χ σ)
                   ((,σ) . Address3)
                   (Map.foldlWithKey' acc Nothing σ)
  where acc Nothing α χ' = if χ == χ' then Just α else Nothing
        acc (Just α) _ _ = Just α
        α = maybe 0 ((+1) . fst) (Map.lookupMax σ)
